...
public class DeckServiceImpl implements DeckService {
     ...
     *
     * Логика работы:
     * 1. Проверяет кеш в Redis по ключу user:{userId}:deck_repeat.
     * 2. Если данные есть — возвращает их.
     * 3. Если данных нет:
     *    - Загружает пользователя из БД, чтобы узнать лимит слов.
     *    - Получает из репозитория список слов для повторения.
     *    - Пытается сгенерировать примеры предложений через Ollama.
     *    - Добавляет примеры к словам (если генерация удалась).
     * 4. Сохраняет колоду в Redis на 12 часов.
     *
